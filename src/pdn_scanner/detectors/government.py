...
from pdn_scanner.normalize import extract_context_window, normalize_numeric_id, normalize_whitespace
...
PASSPORT_COMBINED_RE = re.compile(
    r"(?:(?:паспорт(?:\s*рф)?|passport)[^0-9]{0,25})?"
    r"(?:серия\s*[:#]?\s*|passport series\s*[:#]?\s*)?"
    r"(\d{2}\s?\d{2})"
    r"(?:[^\d]{0,12}(?:номер|number|no\.?|№)\s*[:#]?\s*|[^\d]{0,6})"
    r"(\d{6})",
    flags=re.IGNORECASE,
)
...
PASSPORT_NUMBER_RE = re.compile(
    r"(?:номер(?:\s+паспорта)?|passport number|passport\s+no\.?|passport\s*#)\s*[:#]?\s*(\d{6})",
    flags=re.IGNORECASE,
)
...
DRIVER_LICENSE_CONTEXT_PATTERNS = (
    r"водительское удостоверение",
    r"вод\.\s*удостоверение",
    r"driver license",
    r"driving license",
    r"driving licence",
    r"\bву\b",
...
def _detect_passport(chunk: str, lowered: str, index: int, config: AppConfig) -> list[DetectionResult]:
    ...
    for match in PASSPORT_COMBINED_RE.finditer(chunk):
        ...
        context_end = min(len(chunk), match.end() + 12)
        context = chunk[context_start:context_end].lower()
        passport_keywords = [keyword for keyword in PASSPORT_KEYWORDS if keyword in context]
        has_passport_context = "паспорт" in context or "passport" in context or ("серия" in context and "номер" in context)
...
def _detect_driver_license(chunk: str, lowered: str, index: int, config: AppConfig) -> list[DetectionResult]:
    keywords = [pattern for pattern in DRIVER_LICENSE_CONTEXT_PATTERNS if re.search(pattern, lowered, flags=re.IGNORECASE)]
    if not keywords:
        return []
    ...
    for match in DRIVER_LICENSE_RE.finditer(chunk):
        ...
        detections.append(
            build_detection(
                ...
                context_keywords=["driver_license_context"],
            )
        )
    return detections
